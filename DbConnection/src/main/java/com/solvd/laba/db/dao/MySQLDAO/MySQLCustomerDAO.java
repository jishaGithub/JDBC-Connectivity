...
public class MySQLCustomerDAO extends AbstractDAO<Customer> {
    ...
    public Boolean findById(int id1) {
       ...
       try (PreparedStatement preparedStatement = getConnection().prepareStatement("SELECT * FROM customer where id=?")) {
    ...
    public ArrayList<Customer> selectAll() {
        ...
        try (PreparedStatement preparedStatement = getConnection().prepareStatement("SELECT * FROM customer")) {
            ResultSet result = preparedStatement.executeQuery();
            while (result.next()) {
                int id = result.getInt("id");
                String firstName = result.getString("first_name");
