...
public class CustomerAuthenticationDAO extends AbstractDAO implements GenericDAO<CustomerAuthentication> {
    private static final Logger logger = LogManager.getLogger(CustomerAuthenticationDAO.class);
    ...
    public CustomerAuthentication get(int id) {
        ...
        try (PreparedStatement preparedStatement = getConnection().prepareStatement(GET_BY_ID)) {
            ...
            if (result.next()) {
                ...
                CustomerAuthentication customerAuthentication = new CustomerAuthentication(authId, userName, password, customerId);
