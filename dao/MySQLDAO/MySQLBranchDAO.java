...
public class MySQLBranchDAO extends AbstractDAO<Branch> {
    ...
    public Boolean findById(int id1) {
        ...
        try (PreparedStatement preparedStatement = getConnection().prepareStatement("SELECT * FROM branch where id=?")) {
    ...
    public ArrayList<Branch> selectAll() {
            ...
            try (PreparedStatement preparedStatement = getConnection().prepareStatement("SELECT * FROM branch")) {
                ResultSet result = preparedStatement.executeQuery();
                while (result.next()) {
                    int branchId = result.getInt("id");
                    String branchName = result.getString("branch_name");
